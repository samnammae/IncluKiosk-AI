...
last_facemesh_ts = 0.0

# 디버그/그림 토글 (성능↑ 원하면 끄세요)
DRAW_LANDMARKS = False      # 468개 점 그리기 비활성화 권장
DRAW_DEBUG_VIEW = False     # render_debug_view_orbit 비활성화 권장
...
fps_ema = None
timers = {}
perf_hist = defaultdict(lambda: deque(maxlen=120))  # 최근 120샘플 평균

# (선택) 시스템 리소스: psutil이 있으면 CPU/RAM도 표시
try:
    import psutil
except ImportError:
    psutil = None
    
# capacity/frequency parameters


# ===== EdgeTPU face detector 설정 =====
USE_TPU_FACE = True  # 라즈베리파이에서 True. (윈도우/비지원 환경이면 자동 비활성)
TPU_MODEL_PATH = "/home/pi/IncluKiosk/face-detector-quantized_edgetpu.tflite"  # 네 경로로 수정

tpu_interpreter = None
tpu_input_size = None  # (h, w)
last_face_bbox = None  # (x0,y0,x1,y1) in pixels
last_face_time = 0.0
...
def init_tpu_face_detector():
    ...
    if not USE_TPU_FACE:
        return False
    if platform.system() != "Linux":  # EdgeTPU는 Linux에서 사용
        return False
    try:
        ...
        print(f"[TPU] Face detector ready. input={tpu_input_size}")
        return True
    except Exception as e:
        print("[TPU] init failed -> fallback CPU:", e)
        tpu_interpreter = None
        return False
...
def tpu_face_detect_bboxes_bgr(frame_bgr):
    """
    TPU 얼굴검출 실행.
    returns: [(x0,y0,x1,y1,score), ...] (픽셀 좌표, full-frame 기준)
    """
    ...
    for t in tensors:
        s = t.shape
        # SSD postprocess 표준: boxes [1,N,4], scores [1,N], ...
        if len(s) == 3 and s[2] == 4:
            boxes = t[0]    # [N,4] (ymin, xmin, ymax, xmax) normalized
...
def to_global_landmarks(landmarks, roi, full_w, full_h):
    """
    ROI 기준 정규화 좌표 -> 전체 프레임 기준 정규화 좌표로 변환
    (face_landmarks[i].x * w, .y * h 라는 네 기존 연산에 맞춤)
    """
    x0, y0, x1, y1 = roi
    cw = x1 - x0; ch = y1 - y0
    out = []
...
def timeit_start(name: str):
    timers[name] = time.perf_counter()

def timeit_end(name: str):
    t0 = timers.get(name, None)
    if t0 is None: 
        return 0.0
    dt = time.perf_counter() - t0
    perf_hist[name].append(dt)
    return dt

def avg_ms(name: str):
    q = perf_hist.get(name, [])
    if not q:
        return 0.0
    return 1000.0 * (sum(q) / len(q))

def draw_hud(img, lines, org=(10, 24), line_h=20, font=cv2.FONT_HERSHEY_SIMPLEX,
             font_scale=0.55, color=(0,255,0), thick=1, bg=True):
    """왼쪽 상단에 텍스트 스택을 그려주는 간단 HUD"""
    x, y = org
    if bg:
        # 배경 박스 (가독성↑)
        w = 0
        for s in lines:
            (tw, th), _ = cv2.getTextSize(s, font, font_scale, thick)
            w = max(w, tw)
        h = line_h * len(lines) + 6
        cv2.rectangle(img, (x-6, y-18), (x-6 + w + 12, y-18 + h), (0,0,0), -1)
        cv2.rectangle(img, (x-6, y-18), (x-6 + w + 12, y-18 + h), (40,40,40), 1)
    for i, s in enumerate(lines):
        yy = y + i * line_h
        cv2.putText(img, s, (x, yy), font, font_scale, color, thick, cv2.LINE_AA)
...
while cap.isOpened():
    ...
    if roi_valid:
        ...
        # 풀프레임 FaceMesh 금지(부하 큼). 허용하려면 ALLOW_FALLBACK_FULLFRAME=True
        if ALLOW_FALLBACK_FULLFRAME:
    ...
    if results and results.multi_face_landmarks:
        ...
        last_facemesh_ts = time.time()

        # ===== 시각화(가볍게): 랜드마크 점 찍기 옵션 =====
        if DRAW_LANDMARKS:
        ...
        # ===== 눈-구/시선선 그리기/마우스업데이트 =====
        # 이미 기존 코드의 긴 블록이 있는데, 그 블록은
        # 아래 전제(필요 변수 준비됨)로 그대로 사용 가능:
        # - head_center, R_final, nose_points_3d
        # - iris_3d_left, iris_3d_right
        # 주의: left_sphere_locked, right_sphere_locked 여부에 따라 기존 분기 유지
        # (기존 네 시각화/마우스 갱신 블록은 그대로 두되, 위 변수들이
        #  이 if 내부에서 정의된다는 점만 보장되면 됩니다)

    # ===== Debug orbit view (옵션) =====
    if DRAW_DEBUG_VIEW and (last_head_center is not None):
        render_debug_view_orbit(
            h, w,
            head_center3d=last_head_center,
            ...
            iris3d_l=last_iris_3d_left,
            iris3d_r=last_iris_3d_right,
            left_locked=left_sphere_locked,
            right_locked=right_sphere_locked,
            landmarks3d=last_nose_points_3d if last_nose_points_3d is not None else None,
    ...
    # ROI 크기 표시 (ROI가 있을 때만)
    roi_str = "-"
    if (last_face_bbox is not None) and (time.time() - last_face_time <= FACE_TTL):
        x0, y0, x1, y1 = expand_and_clip_bbox(last_face_bbox, ROI_MARGIN, w, h)
        roi_str = f"{x1-x0}x{y1-y0}"

    # CPU/RAM
    cpu_ram = ""
    if psutil:
        cpu_ram = f" | CPU {psutil.cpu_percent():.0f}%  MEM {psutil.virtual_memory().percent:.0f}%"

    hud_lines = [
        f"FPS {fps_ema:.1f}" if fps_ema else "FPS ...",
        f"TPU detect {avg_ms('tpu_detect'):.1f} ms",
        f"FaceMesh {avg_ms('facemesh'):.1f} ms",
    ...
    if key == ord('q'):
        ...
        # 최근 FaceMesh 결과가 준비되어 있는지 확인
        if (last_head_center is None or last_R_final is None or
            last_nose_points_3d is None or last_iris_3d_left is None or last_iris_3d_right is None):
            print("[Calib] 최근 FaceMesh 결과 없음. 얼굴을 프레임에 두고 잠시 후 다시 C.")
        else:
            ...
            monitor_corners, monitor_center_w, monitor_normal_w, units_per_cm = create_monitor_plane(
                last_head_center, last_R_final, face_landmarks if face_landmarks else [], w, h,
        ...
        if (last_head_center is None or last_R_final is None or
            last_nose_points_3d is None or last_iris_3d_left is None or last_iris_3d_right is None):
            print("[Screen Calib] 최근 FaceMesh 결과 없음. 잠시 후 다시 S.")
        else:
            # 현재 스케일에 맞춘 구 위치 재구성
            current_nose_scale = compute_scale(last_nose_points_3d)
...
if not cap.isOpened():
    print("[ERROR] Camera open failed. /dev/video0 가 있는지, 레거시 카메라가 켜졌는지 확인하세요.")
    exit(1)
