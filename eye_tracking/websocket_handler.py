...
class WebSocketHandler:
    ...
    async def send_message(self, payload: dict):
        ...
        try:
            async with websockets.connect(config.WS_URL) as ws:
                ...
                self._log(f"[eye_worker WS] sent {payload}")
        except Exception as e:
            self._log(f"[eye_worker WS] send failed: {e}")
    ...
    async def _receiver(self):
        ...
        while True:
            try:
                self._log("[eye_worker WS] trying to connect...")
                async with websockets.connect(config.WS_URL) as ws:
                    self._log("[eye_worker WS] connected")
                    
                    while True:
                        raw = await ws.recv()
                        self._log(f"[eye_worker WS] recv raw={raw}")
                        ...
                        self._log(f"[eye_worker WS] parsed type={msg_type}")
                        self._handle_message(msg_type, data)
                        
            except Exception as e:
                self._log(f"[eye_worker WS] connect failed/disconnected: {e} (retry in 2s)")
    ...
    def _handle_message(self, msg_type, data):
        ...
        if msg_type == "EYE_CALIB_ON":
            ...
            self._log("[eye_worker WS] TOUCH_ACTIVE")
            if self.on_touch_active:
                self.on_touch_active()
        
        elif msg_type == "TOUCH_IDLE":
            self._log("[eye_worker WS] TOUCH_IDLE")
    ...
    def start_receiver(self):
        ...
        if self.receiver_running:  # 이미 실행 중이면 무시
            self._log("[eye_worker WS] receiver already running")
            return
        ...
        thread.start()
        self._log("[eye_worker WS] receiver thread starting...")