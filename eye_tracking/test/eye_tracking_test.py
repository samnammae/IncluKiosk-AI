"""
아이트래킹 (캘리브레이션(c) - 중앙화(s) - 마우스제어&클릭(f7))
기능만 빼놓은 테스트 코드

실행방법
sudo /home/pi/IncluKiosk/IncluKiosk_venv/bin/python eye_tracking_test.py

로컬 파이썬이 아닌 가상환경 파이썬을 쓰는 것이 매우매우매우매우 중요
"""
...
clicker = ClickController(
    prepare_time=0.40,   # 파란 링 준비 단계 시작
    progress_time=0.80,  # 초록 링 진행 단계 시작
    click_time=1.20,     # 클릭 실행 시점
    radius=40,           # 허용 반경(px)
    cooldown=0.50        # 클릭 후 쿨다운
)
...
def convert_gaze_to_screen_coordinates(combined_gaze_direction, calibration_offset_yaw, calibration_offset_pitch):
    ...
    yawDegrees = config_test.YAW_SENSITIVITY  # x degrees left or right # 좌우 한계(도)
    pitchDegrees = config_test.PITCH_SENSITIVITY  # x degrees up or down # 상하 한계(도)
...
while cap.isOpened():
    ...
    if results.multi_face_landmarks:
        ...
        if left_sphere_locked and right_sphere_locked:
            ...
            now = time.time()
            if mouse_control_enabled:
                status = clicker.update((screen_x, screen_y), now)
            else:
                status = clicker.update(None, now)  # 이동 off면 내부 상태 리셋
                
            # 클릭 실행
            if status.get('should_click'):
                cx, cy = status['center']
                # 마우스가 이미 이동 스레드로 따라오고 있다면 좌표 지정 없이 click()만 호출해도 충분
            ...
            if state in ('prepare', 'progress') and center is not None:
                # 진행도에 따라 반지름을 키우고 색을 바꿈
                ring_r = int(12 + 12 * progress)  # 12~24px
                color = (255, 120, 0) if state == 'prepare' else (0, 220, 0)  # 파란 느낌/초록 느낌
                cv2.circle(frame, (int(center[0]), int(center[1])), ring_r, color, 2)
            elif state == 'click' and center is not None:
                cv2.circle(frame, (int(center[0]), int(center[1])), 26, (0, 255, 255), 3)
    ...
    if keyboard.is_pressed('f7'):
        ...
        clicker.set_enabled(mouse_control_enabled) 
